...
class CharacterVO extends ValueObject {
    ...
    public function __construct(array $char) {
        ...
        $this->updatedAt = $char['updatedAt'];

        $this->name = $char['name'];

        $this->level = $char['level'];
        $this->experience = $char['experience'];
        $this->experienceToLevel = $char['experienceToLevel'];

        $this->hitPoints = $char['hitPoints'];
        $this->manaPoints = $char['manaPoints'];

        $this->silver = $char['silver'];
        $this->gold = $char['gold'];
        $this->gems = $char['gems'];
        $this->coins = $char['coins'];

        $this->maxBagSlots = $char['maxBagSlots'];
        $this->maxBankSlots = $char['maxBankSlots'];
        $this->maxHouseSlots = $char['maxHouseSlots'];
        $this->maxHouseItemSlots = $char['maxHouseItemSlots'];

        $this->hasDragonAmulet = $char['hasDragonAmulet'] != 0;
        $this->accessLevel = $char['accessLevel'];

        $this->gender = $char['gender'];
        $this->pronoun = $char['pronoun'];

        $this->hairId = $char['hairId'];
        $this->colorHair = \hexdec($char['colorHair']);
        $this->colorSkin = \hexdec($char['colorSkin']);
        $this->colorBase = \hexdec($char['colorBase']);
        $this->colorTrim = \hexdec($char['colorTrim']);

        $this->questId = $char['questId'];
        ...
        $this->skillPoints = $char['skillPoints'];
        $this->statPoints = $char['statPoints'];

        $this->status = $char['status'];
        $this->daily = $char['daily'];

        $this->armor = $char['armor'];
        $this->skills = $char['skills'];
        $this->quests = $char['quests'];

        $this->raceId = $char['raceId'];
        $this->classId = $char['classId'];
        $this->baseClassId = $char['baseClassId'];
    }
